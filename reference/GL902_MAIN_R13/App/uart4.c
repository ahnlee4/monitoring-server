...
void USART4_ISR(void)
{
    ...
    else if( Uart_Info[UART4_485].Rx_Cnt >= 2 )
    {
        if( Uart_Info[UART4_485].Rx_Buf[1] == 0x13 )
        {
            ...
            {
                Uart_Info[UART4_485].rcv_length = (Uart_Info[UART4_485].Rx_Buf[4] << 8 | Uart_Info[UART4_485].Rx_Buf[5]);

                Uart_Info[UART4_485].Rx_Buf[Uart_Info[UART4_485].Rx_Cnt++] = RxData;

                if( Uart_Info[UART4_485].Rx_Cnt == (6 + Uart_Info[UART4_485].rcv_length + 2) )		// 헤더 + 데이터 + CRC
        ...
        else if( Uart_Info[UART4_485].Rx_Buf[1] == 0x15 )
        {
            ...
            {
                Uart_Info[UART4_485].rcv_length = (Uart_Info[UART4_485].Rx_Buf[2] << 8 | Uart_Info[UART4_485].Rx_Buf[3]);

                Uart_Info[UART4_485].Rx_Buf[Uart_Info[UART4_485].Rx_Cnt++] = RxData;

                if( Uart_Info[UART4_485].Rx_Cnt == (4 + Uart_Info[UART4_485].rcv_length + 2) )		// 헤더 + 데이터 + CRC
                    Uart_Info[UART4_485].Rcv_Pkt = SET;
            }
            else
            {
        ...
        else if( Uart_Info[UART4_485].Rx_Buf[1] == 0x03 )
        {
            ...
            {
                Uart_Info[UART4_485].Rx_Buf[Uart_Info[UART4_485].Rx_Cnt++] = RxData;
                Uart_Info[UART4_485].rcv_length = RxData;
            }
            else if( Uart_Info[UART4_485].Rx_Cnt >= 3 )
            {
                Uart_Info[UART4_485].Rx_Buf[Uart_Info[UART4_485].Rx_Cnt++] = RxData;

                if( Uart_Info[UART4_485].Rx_Cnt == (3 + Uart_Info[UART4_485].rcv_length + 2) )
                    Uart_Info[UART4_485].Rcv_Pkt = SET;
            }
        }
        else
...
void Uart4_Tx_Process(void)
{
    ...
    if( Uart_Info[UART4_485].Tx_Timeout == 1 )      // 응답없음. 연결비트 클리어.
    {
        ...
        if( Uart_Info[UART4_485].Ack_Flag == CLR )
        {
            if( Uart_Info[UART4_485].Call_Id >= MEM_ADDR_COMP1 && Uart_Info[UART4_485].Call_Id <= MEM_ADDR_COMP8 )
            {
                if( Main_Info.Comp_Connect_Cnt[Uart_Info[UART4_485].Call_Id - MEM_ADDR_COMP1] > 0 )     
			Main_Info.Comp_Connect_Cnt[Uart_Info[UART4_485].Call_Id - MEM_ADDR_COMP1]--;
            }
        }
    }
