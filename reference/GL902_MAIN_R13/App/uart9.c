...
extern UART_HandleTypeDef huart9;       // 외부기기제어. 모드버스.

void USART9_ISR(void)
{
    unsigned short rcv_length;
    unsigned char RxData;

    if( Uart_Info[UART9_LTE].Rx_Timeout == 0 )   Uart_Info[UART9_LTE].Rx_Cnt = 0;
    if( Uart_Info[UART9_LTE].Rx_Cnt > 255 )   Uart_Info[UART9_LTE].Rx_Cnt = 0;
    
    RxData = UART9->DR;

    if(Uart_Info[UART9_LTE].Rx_Timeout == 0 && RxData == 0x01 )			// ID 검사
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt == 1 && (RxData == 0x10 || RxData == 0x03 || RxData == 0x06) )					// 쓰기, 읽기 명령
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt == 2 )					// 상위 주소
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt == 3 )					// 하위 주소
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt == 4 )					// 데이터 길이
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt == 5 )					// 데이터 길이
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt >= 6 && Uart_Info[UART9_LTE].Rx_Buf[1] == 0x03 )	// 읽기
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;

        if( Uart_Info[UART9_LTE].Rx_Cnt == 8 )
            Uart_Info[UART9_LTE].Rcv_Pkt = SET;
    }
    else if( Uart_Info[UART9_LTE].Rx_Cnt >= 6 && Uart_Info[UART9_LTE].Rx_Buf[1] == 0x06 )	// 쓰기
    {
        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;
    ...
    else if( Uart_Info[UART9_LTE].Rx_Cnt >= 6 && Uart_Info[UART9_LTE].Rx_Buf[1] == 0x10 )	// 쓰기
    {
        rcv_length = Uart_Info[UART9_LTE].Rx_Buf[6];

        Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt++] = RxData;

        if( Uart_Info[UART9_LTE].Rx_Cnt == (7 + rcv_length + 2) )		// 헤더 + 데이터 + CRC
            Uart_Info[UART9_LTE].Rcv_Pkt = SET;
    }
    else
    {
        Uart_Info[UART9_LTE].Rx_Cnt = 0;
    }

    Uart_Info[UART9_LTE].Rx_Timeout = 10;
}

void Uart9_Rcv_0x03_Data(unsigned char *pData)
{
    ...
    else if( upper_addr == ADDR_LTE_MEMORY || upper_addr == ADDR_LTE_MEMORY_256 || upper_addr == ADDR_LTE_MEMORY_512 )		//LTE
    {
        ...
        Uart_Info[UART9_LTE].Tx_Buf[0] = pData[0];
        Uart_Info[UART9_LTE].Tx_Buf[1] = pData[1];
        
        p = &Uart_Info[UART9_LTE].Tx_Buf[3];
		
        Uart_Info[UART9_LTE].Tx_Length = 0;
        ...
        for(i=0;i<pData[5];i++)
        {
            ...
            *p++ = *pWord & 0xFF;
            ...
            pWord++;
            //lower_addr++;
            Uart_Info[UART9_LTE].Tx_Length += 2;
        }
        
        Uart_Info[UART9_LTE].Tx_Buf[2] = Uart_Info[UART9_LTE].Tx_Length;
        
        Uart_Info[UART9_LTE].Tx_Length += 3;            // 헤더 3바이트
        
        cal_crc = crc16(&Uart_Info[UART9_LTE].Tx_Buf[0],Uart_Info[UART9_LTE].Tx_Length);
...
//        HAL_UART_Transmit_IT(&huart9,Uart_Info[UART9_LTE].Tx_Buf,Uart_Info[UART9_LTE].Tx_Length);
}

void Uart9_Rcv_0x10_Data(unsigned char *pData)
{
    unsigned short word_buf, *pWord, *pWord2, word_cnt, byte_cnt, i, cnt, addr, cal_crc;
    unsigned char upper_addr, lower_addr, *p;
...
void Uart9_Rcv_0x06_Data(unsigned char *pData)
{
    unsigned short word_buf, *pWord, *pWord2, word_cnt, byte_cnt, i, cnt, addr, cal_crc;
    unsigned char upper_addr, lower_addr, *p;
...
*/				
	...
	Uart_Info[UART9_LTE].Tx_Buf[0] = pData[0];
	Uart_Info[UART9_LTE].Tx_Buf[1] = pData[1];
	Uart_Info[UART9_LTE].Tx_Buf[2] = pData[2];
	Uart_Info[UART9_LTE].Tx_Buf[3] = pData[3];
	Uart_Info[UART9_LTE].Tx_Buf[4] = pData[4];
	Uart_Info[UART9_LTE].Tx_Buf[5] = pData[5];

	cal_crc = crc16(&Uart_Info[UART9_LTE].Tx_Buf[0],6);

	Uart_Info[UART9_LTE].Tx_Buf[6] = cal_crc >> 8;
	Uart_Info[UART9_LTE].Tx_Buf[7] = cal_crc;

	Uart_Info[UART9_LTE].Tx_Length = 8;      // 헤더 + 데이터 +  CRC
	
//        HAL_UART_Transmit_IT(&huart9,Uart_Info[UART9_LTE].Tx_Buf,Uart_Info[UART9_LTE].Tx_Length);
}


void Uart9_Tx_Process(void)
{
    if( Uart_Info[UART9_LTE].Tx_Timeout > 1 )   return;
    
    if( Uart_Info[UART9_LTE].Tx_Timeout == 1 )
    {
        Uart_Info[UART9_LTE].Tx_Timeout = 0;
        
        Uart_Info[UART9_LTE].Tx_On = SET;
...
void Uart9_Rx_Process(void)
{
    unsigned short cal_crc, rcv_crc;
    
    if( Uart_Info[UART9_LTE].Rcv_Pkt == SET )
    {
        Uart_Info[UART9_LTE].Rcv_Pkt = CLR;
    
        cal_crc = crc16((unsigned char*)Uart_Info[UART9_LTE].Rx_Buf, Uart_Info[UART9_LTE].Rx_Cnt - 2);
        rcv_crc = Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt - 2];
        rcv_crc <<= 8;
        rcv_crc |= Uart_Info[UART9_LTE].Rx_Buf[Uart_Info[UART9_LTE].Rx_Cnt - 1];

        if( cal_crc == rcv_crc )                            // 패킷수신.
        {
	      	Uart_Info[UART9_LTE].Link_Time = 3000;

            if( Uart_Info[UART9_LTE].Rx_Buf[1] == 0x03 )           // 0x03 수신. 멀티리드
            {
                Uart9_Rcv_0x03_Data(&Uart_Info[UART9_LTE].Rx_Buf[0]);
            }
            else if( Uart_Info[UART9_LTE].Rx_Buf[1] == 0x10 )      // 0x10 수신. 멀티라이트
            {
                Uart9_Rcv_0x10_Data(&Uart_Info[UART9_LTE].Rx_Buf[0]);
            }
            
            else if( Uart_Info[UART9_LTE].Rx_Buf[1] == 0x06 )      // 0x06 수신. 싱글라이트
            {
                Uart9_Rcv_0x06_Data(&Uart_Info[UART9_LTE].Rx_Buf[0]);
                Uart_Info[UART9_LTE].Rx_Cnt = 0;
            }
	    else return;
            
            Uart_Info[UART9_LTE].Rx_Timeout = 0;
            Uart_Info[UART9_LTE].Tx_Timeout = 5;                   // 패킷수신 후 다음번 명령까지의 TX 딜레이는 상대측 보드의 내부 지연시간을 고려해서 50ms 후에 전송함.
        }
    }
}
